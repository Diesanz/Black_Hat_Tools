import argparse
import socket
import shlex #evitar problemas a la hora de romper cadena
import subprocess
import sys
import textwrap
import threading
...
class NetCat:
    def __init__(self, args, buffer=None):
        self.args = args #initialize de NetCat obj with args
        self.buffer = buffer
        self.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)

    def run(self): #entry point to manage netcat
        if self.args.listen:
            self.listen()
        else:
            self.send()
...
#receives a comand, runs it, returns de output as a str
def execute(cmd):
    cmd = cmd.strip()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--port', type=int, default=5555, help='specified a port')
    parser.add_argument('-t', '--target', default='192.168.1.203', help='specific IP')
    parser.add_argument('-u', '--upload', help='upload file')

    args = parser.parse_args()

    if args.listen:#setting up as a listener
        buffer = ''
    else:
